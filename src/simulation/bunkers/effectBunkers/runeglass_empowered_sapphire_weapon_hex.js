/**
 * Runeglass of Empowered Sapphire (Weapon) - Hex Effect
 * Hits inflict Hex, dealing 8% Arcane damage per second for 2s.
 */

const METADATA = {
  id: 'runeglass_empowered_sapphire_weapon_hex',
  name: 'Runeglass of Empowered Sapphire (Weapon) - Hex',
...
function handler({ event, source, target, timestamp }) {
  ...
  console.log('[Runeglass Empowered Sapphire] 🔮 Applying Hex DoT');

  // Apply Hex: 8% weapon damage per second for 2 seconds
  return {
    applyEffects: [
      {
        id: 'runeglass_sapphire_hex',
        category: 'DOT',
        subtype: 'HEX',
        sourceId: source.id,
        targetId: target.id,
