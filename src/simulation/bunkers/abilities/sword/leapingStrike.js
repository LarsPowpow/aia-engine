 ...
 * @version 4.1.0 - "STROKE 1 DATA" Diagnostic Probe
 */
...
const twoStrokeProcessEvent = (event, currentCombatants, allSources, addRawLog, analysisLog) => {
    ...
    addRawLog({ level: 'info', message: `Processing event: ${event.abilityId || 'N/A'}`, event });
    ...
    let damageTerms = { empowerPercent: 0, rendPercent: 0, baseCritDmgPercent: 0, positionalDmgPercent: 0, miscDmgPercent: 0 };
    damageTerms = stateManager.applyDamageModifiers(damageTerms, modifierRequests);
    
    // --- NEW DIAGNOSTIC PROBE 3 ---
    console.log('%c[STROKE 1 DATA]', 'color: #cyan; font-weight: bold;', {
        ability: event.abilityId || event.action,
        requests: modifierRequests,
        finalTerms: damageTerms
    });
    
    // --- DAMAGE CALCULATION ---
    const finalDamage = calculateFinalDamage(context, modifierRequests, damageTerms);
