    ...
    return (
        <div className="bg-slate-800/40 rounded-xl p-4 flex flex-col space-y-4 border border-slate-700 shadow-lg backdrop-blur-sm">
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                <label className="text-sm font-medium text-slate-400 mb-2 block">Weapon</label>
                <select 
                    value={weaponType}
                    onChange={(e) => setWeaponType(e.target.value)}
                    className="w-full bg-slate-700 border border-slate-600 text-white rounded-md p-2 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition"
                >
                    ...
                    {Object.keys(attributes).map(attr => (
                        <div key={attr} className={attr === 'CON' ? 'col-span-2' : ''}>
                            ...
                            <input 
                                ...
                                className="w-full bg-slate-700/80 border border-slate-600 rounded-md p-2 mt-1 focus:bg-slate-600 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition"
                            />
                        </div>
                    ))}
                </div>
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg border border-amber-500/30 text-center shadow-inner">
                 ...
                 <div className="flex justify-center items-center">
                     <span className="text-4xl font-mono font-bold text-white bg-slate-800/50 px-4 py-1 rounded-md shadow-[0_0_8px_rgba(251,191,36,0.2)]">{calculatedDamage}</span>
                 </div>
            </div>
        </div>
    );
...
const CombatAnalysisPanel = ({ combatLog, isFocusMode, setIsFocusMode }) => {
    ...
    const getInspectorData = (entry) => {
        return {
            combatantState: { health: '100/100', stamina: '50/100', buffs: entry.activeBuffs || 'None' },
            targetState: { health: '45000/50000', debuffs: entry.effectsApplied || 'None' },
            formulaBreakdown: `Base: 500 * (1 + STR Bonus: 0.5) * (1 - Armor Mit: 0.2) = ${entry.damage}`
        };
    };
    ...
    return (
        <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700 flex flex-col h-full shadow-lg backdrop-blur-sm">
            <div className="flex justify-between items-center border-b border-slate-600 pb-2 mb-2">
                <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-violet-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a1 1 0 001 1h12a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1-1zm2 4a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1zm1 4a1 1 0 100 2h3a1 1 0 100-2H6z" clipRule="evenodd" /></svg>Combat Analysis</h2>
                <div className="flex items-center space-x-2">
                    <button 
                        onClick={() => setIsFocusMode(!isFocusMode)}
                        className="p-2 rounded-md hover:bg-slate-700 text-slate-400 hover:text-white transition" title="Toggle Focus Mode"
                    >
            ...
            <div className="flex-grow overflow-auto custom-scrollbar pr-1">
                 <table className="min-w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                        {combatLog.length === 0 && ( <tr><td colSpan="6" className="text-center text-slate-500 py-16">Run a simulation to see the results.</td></tr>)}
                        {combatLog.map((entry, index) => (
                             <tr key={index} className="hover:bg-slate-700/50 cursor-pointer transition-colors duration-150 even:bg-slate-800/20" onClick={() => handleRowClick(index)}>
    ...
    return (
        ...
        <div className="h-screen flex flex-col p-4 sm:p-6 space-y-4 bg-gradient-to-br from-slate-900 to-slate-800 text-slate-300 font-sans tactical-grid">
            <div className="flex justify-between items-center flex-shrink-0">
                <h1 className="text-2xl font-bold text-amber-400 tracking-wider flex items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M12 6V3m0 18v-3m6-9h3m-18 0h3m15-3l-2 2m-10-2l2 2m-2 10l2-2m10 2l-2-2" />
                    </svg>
                    Combat Simulator
                </h1>
            </div>
            ...
            <div className="flex-shrink-0">
                <CommandBar onRunSimulation={handleRunSimulation} onClearLog={clearCombatLog} isPrimary={true}/>
            </div>
            ...
            <div className={`flex-grow grid gap-6 ${isFocusMode ? 'grid-cols-1' : 'lg:grid-cols-3'} overflow-hidden`}>
                <div className={`${isFocusMode ? 'hidden' : 'lg:col-span-1'} flex flex-col gap-6 overflow-y-auto custom-scrollbar p-1`}>
                    ...
                    <div className="mt-auto pt-4">
                        <CommandBar onRunSimulation={handleRunSimulation} onClearLog={clearCombatLog} isPrimary={false} />
                    </div>
                </div>
